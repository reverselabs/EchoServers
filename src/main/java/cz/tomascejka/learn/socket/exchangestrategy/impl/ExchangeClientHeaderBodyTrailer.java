 ...
 */
public class ExchangeClientHeaderBodyTrailer implements ExchangeStrategy<byte[], byte[]> {
	...
	protected byte[] receiveInternally() throws IOException, ExchangeStrategyException
	{
		...
		if (inputStream.available() >= HEADER_LENGTH)//existuji data s 2 byte delkou (napr. hlavicka)?
		{
			...
			if(header.length < 1)
			{
				LOG.warn("Header is empty, lenght={}. Empty message is returned", header.length);
				return new byte[0];
			}
